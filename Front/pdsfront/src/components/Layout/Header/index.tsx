...
import { headerData, patientMenu, medecinMenu, adminMenu, managerMenu, securityMenu } from "../Header/Navigation/menuData";
...
const Header: React.FC = () => {
  ...
  const [currentMenu, setCurrentMenu] = useState(headerData);
  const [userRole, setUserRole] = useState<string | null>(null);

  useEffect(() => {
    // Check for user role and update menu
    const checkRole = () => {
      ...
      if (userStr) {
        try {
          ...
          // Import menus dynamically if possible or just use the ones we imported
          // Since we need to import them at top level, we'll do that in a separate edit step or assume they are available
          // For now, let's assume we need to add the imports in the file too. 
          // Wait, I need to update imports first.
          // Or I can do it all here if I used multi_replace or big chunk.
          // Let's rely on the user having the imports from the previous step? No, `menuData.tsx` exports them.
          // I'll assume I'm editing the component body here and will fix imports in next step if needed or implicitly.

          /* 
             Wait, I cannot add imports inside the component. 
             I will need to add imports at top of file. 
             This tool call is only replacing the body part.
             So I will use multi_replace to handle both.
          */
        } catch (e) { console.error(e); }
      } else {
        setUserRole(null);
        setCurrentMenu(headerData);
      }
    };

    checkRole();
    window.addEventListener('storage', checkRole); // Listen for login/logout events across tabs or local updates
    // Custom event for same-tab login updates?
    // Dispatching a custom event on login would be better.
          ...
          <div className="flex items-center justify-between py-6">
            ...
            <ul className="hidden lg:flex flex-grow items-center justify-center space-x-6">
              ...
              {(userRole === 'PATIENT' ? patientMenu :
                userRole === 'MEDECIN' ? medecinMenu :
                  userRole === 'ADMIN' ? adminMenu :
                    userRole === 'MANAGER' ? managerMenu :
                      userRole === 'RESPONSABLE_SECURITE' || userRole === 'SECURITY_OFFICER' ? securityMenu : // Handling potential role name variations
